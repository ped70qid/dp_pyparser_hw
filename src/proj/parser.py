...
A slightly modfied version of Honnibal's original implementation."""
from os import path
import os
import io
import sys
...
class PerceptronTagger(object):
    ...
    def __init__(self, model_loc, classes=None, load=True):
...
def read_conll(loc):
    for sent_str in open(loc).read().strip().split('\n\n'):
        ...
        for line in lines:
            (_, word, _, pos, _, _, head, label, _, _) = line 
            words.append(word)
            tags.append(pos)
...
def main_test(model_dir, heldout_gold):
...
def main_parse(model_dir, input_text=None):
    parser = Parser(model_dir, load=True)
    ...
    for line in stream:
        ...
        words = words[1:]
        tags = tags[1:]
        heads = [0 if h == len(heads) else h for h in heads[1:]]
        for i, (word, tag, head) in enumerate(zip(words, tags, heads), start=1):
            print(i, word, '_', tag, '_', '_', head, '_', '_', '_')
        print()
...
if __name__ == '__main__':
    if len(sys.argv) == 4 and sys.argv[1] == 'train':
        ...
        main_parse(sys.argv[2])
    else:
        print('USAGES:', file=sys.stderr)
        print('python3 parser.py train MODEL_DIR TRAIN_DATA', file=sys.stderr)
        print('python3 parser.py test MODEL_DIR VAL_DATA', file=sys.stderr)
        print('python3 parser.py parse MODEL_DIR < tokenized_input_sentences.txt', file=sys.stderr)
        sys.exit(1)
